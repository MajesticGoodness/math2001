...
example {a b m n : ℤ} (h1 : a * m + b * n = 1) (h2 : b ^ 2 = 2 * a ^ 2) :
  ...
  calc
    ...
    _ = 4 * (a ^ 2) * (n ^ 2) + (4 * a * n * b * m) + 2 * (a ^ 2) * (m ^ 2) := by rw [h2]
    _ = 2 * (2 * (a ^ 2) * (n ^ 2) + 2 * a * n * b * m + (a ^ 2) * (m ^ 2)) := by ring
    _ = 2 * ((b ^ 2) * (n ^ 2) + 2 * a * n * b * m + (a ^ 2) * (m ^ 2)) := by rw [h2]
    _ = 2 * ((a ^ 2) * (m ^ 2) + 2 * (a * m * b * n) + (b ^ 2) * (n ^ 2)) := by ring
